...
p7+geom_histogram()+
  labs(title="Histogram of Project Duration",
       x="Duration",
       y="Count")
```
...
```{r}
p8 <- ggplot(ks_filt, aes(x = duration))

p8 + geom_histogram(aes(fill = state)) + 
labs(title = "Histogram of Project Duration by State",
x = "Duration",
y = "Count",
fill = "State") +
facet_grid(~state)
...
p9<-ggplot(back_filt,aes(x=backers))
p9+geom_histogram()+ 
labs(title = "Histogram of Project Backers",
x = "Backers",
y = "Count")
```
...
```{r}
p10 <- ggplot(back_filt, aes(x = backers,fill=state))

p10 + geom_density(alpha=.4) + 
labs(title = "Density of Project Backers by State",
x = "Backers",
y = "Density",
fill = "State")
```
...
p11 <- ggplot(ks_goal, aes(x = usd_goal_real))

p11 + geom_histogram() + 
labs(title = "Histogram of Project Goal (USD) by State",
x = "Goal",
y = "Count")
```
...
```{r}
p11+ geom_density(aes(fill = state), alpha = 0.4)+ 
labs(title = "Density of Project Goal (USD) by State",
x = "Goal (USD)",
y = "Density",
fill = "State")
```
...
p12 <- ggplot(pledge_filt, aes(x = usd_pledged_real))

p12 + geom_histogram()+ 
labs(title = "Histogram of Amount Pledged to Project (USD)",
...
```{r}
p14<-ggplot(ks_goal,aes(x=usd_goal_real, fill=state))

p14 + geom_histogram() + facet_wrap(~main_category)+
  labs(title = "Project Goal (USD) by State across Category",
       x="Goal (USD)",
       y="Count")
```
...
p13+geom_boxplot()+
  labs(title = "Boxplot of Amount Pledged by Project State and Continent",
       x="Continent",
...
backers_pledged <- ggplot(ks_rand, aes(x = backers, y = usd_pledged_real,alpha=.1))

backers_pledged + geom_point(aes(colour = state,size=usd_goal_real))+
  labs(title="Project State by Amount Pledged to Project, Backers and Goal",
       y="Amount Pledged (USD)",
       x="Backers")

...
This project aims to construct a classification model to predict whether a Kickstarter project will be successful or fail. The project is split into two phases. Phase 1 will focus on data preprocessing and exploration and will be covered in this report, while Phase 2 will be concerned with model building and validation and will be covered in a later report. In this phase, the dataset and its attrbiutes will be unpacked and described. The data will then be cleaned and preprocessed. Finally, all relevant attributes will be explored with regards to their relationships with each other.
...
Here, we see that the proportion of complete cases is very high, at 99.94% of all observations. This suggested that the number of observations affected by the typo was minimal and as such, it was deemed appropriate to omit all affected observations from the analysis.
...
Clearly the main point here is that there were more failed projects in the data than there were successful ones. This was considered in visualizations to come.
...
From Figure 2, it can be seen that projects based on film and video as well as music were popular, while dance and journalism based projects were less common. Furthermore, Figure 3 highlights that generally, most categories are associated with failed projects, except for comics, dance, music and theater. Clearly these are all related to the arts.

Next we looked at the currency and country feature simultaneously to identify any redundancy.
...
Inspection of Figures 4 and 5 does highlight a striking similarity. Additionally, they also serve to emphasize that a large proportion of the projects are associated with the US, and also, that many other countries and currencies have much smaller amounts of associated projects. As such, utilizing either of these features may lead to some issues with regards to disproportional levels. It was therefore decided that it would be best to aggregate the levels of the country feature to represent the continents, and to then drop the country and currency features. The plots of the new continent feature are shown below.
...
Figure 9 below depicts the log of the number of backers each projects had and shows that many projects received relatively few backers, while only a minority recived very large amounts. Figure 10 further shows that the distribution is essentially split across the state feature, with failed projects being especially associted with low amounts of backers, and successful ones having much more varience; demonstrating similarity to a normal distribution. 

```{r}
p8<-ggplot(ks_filt,aes(x=log(backers+1)))
p8+geom_histogram(bins=30)+ 
  labs(title = "Figure 9. Histogram of Log Project Backers",
       x = "Log Backers",
       y = "Count")
```
...
p9 + geom_density(alpha=.4) + 
  labs(title = "Figure 10. Density of Log Project Backers by State",
       x = "Log Backers",
       y = "Density",
       fill = "State")+
...
Each project's goal was visualised in Figures 11 and 12. Figure 11 highlights an unsual distribution with spikes at every $5000. This may be attributable to individuals commonly setting goals at these essentially arbitrary points.
...
The log of amount pledged to each project is visualised below in Figures 13 and 14. Here, there is a striking similarity to the visualisations of the backers feature shown in Figures 9 and 10. Again, the overall distribution is almost normal with a large deviation at zero. This deviation is again mostly comprised of of failed projects recieving very little fudning, while the amount pledged to successful projects has considerable variablity.

```{r}
p11 <- ggplot(ks_filt, aes(x = log(usd_pledged_real+1)))

p11 + geom_histogram(bins=30)+ 
  labs(title = "Figure 13. Histogram of Log Amount Pledged to Project",
...
p12 + geom_density(alpha=.4) + 
  labs(title = "Figure 14. Density of Log Amount Pledged to Project by State",
       x = "Log Amount Pledged (USD)",
       y = "Density",
       fill = "State")
```
...
p15 + geom_point(aes(colour = state,size=usd_goal_real),alpha=.3)+
  facet_wrap(~state)+
  labs(title="Figure 17. Project State by Log Amount Pledged to Project, Log Backers and Goal",
       y="Log Amount Pledged (USD)",
       x="Log Backers")

```

There are several interesting points to note about Figure 17. Unsurprisingly, failed projects can be seen to be highly concertrated at very low values of backers and amounts pledged, with some receiving zero of both. The successful projects are much more spread out in the top right portion of the plot. The dispertion of successful points is also promising for the upcoming analysis, as it suggests that there is not too much correspondence between backers and amount pledged. However, the concentration of points does indicate that succesful projects typically receive moderate amounts of backer and funding. This may ultimately be linked to their smaller goals which contrast the much larger goals evident among failed projects. As previously mentioned, this would suggest that perhaps failed projects were too ambitious with their goals, and considering this, it would make sense that the projects with smaller goals were more likely to reach their targets and become successful; requiring fewer backers pledging smaller amounts. 
This plot was constructed using a random sample from the dataset.

# Summary

In this phase of the report, the relevance of each feature in the original Kickstarter dataset was assessed in order to best predict whether a project would fail or succeed. Of the 15 original features, only five were retained, and two new features were created. For the target feature, only observations relating to successful and failed projects were included and others were dropped. ID and name were both descriptive features which would add no insight into the upcoming analysis so were dropped. The pledged, goal and usd_pledged features were all dropped as they contained ultimately redundant information. Deadline and launched were combined to produce the duration feature, and then were both dropped. The category feature was also dropped as it was found to be too specific and therefore redunadant. Finally, some typos were found within the country feature and all irrelevant observations were dropped. 

Visual assessment of the remaing features further demonstrated the usefulness of some features. Namely, it was shown that the cardinality of the country feature was too high given the low counts of projects within some coutnries and as such, the country feature was dropped and converted to represent continent. Additionaly the duration feature was found to add little insight into the distinction between failed and successful projects, despite this it has been retained for further evaluation in Phase 2. Important insights drawn from visualising remaining features included how projects with smaller, realistic goals are more likely to succeed, however this is not strictly true for all categories - some of which, such as those related to arts, are highly successful regardless of goal. It was found that North American projects were the most common and also most likely to be successful and that Asian projects, which were least common, received the relatively most funding. Finally, it was found that although successful projects are unsuprisingly more likely to have more backers and funding than failed ones, the relationship between these features suggests most succesful projects receive relatively few backers who pledge moderate amounts, likely in accordance with their smaller goals.